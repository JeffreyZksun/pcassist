...
CString CXmlNodeWrapper::GetValue(LPCTSTR valueName)
{
	...
	if (attribute)
	{
		return (CString)(LPCSTR)attribute->Gettext();
	}
	return _T("");
}
...
{
	if (IsValid())
		return (CString)(LPCSTR)m_xmlnode->Getxml();
	else
		return _T("");
}
...
{
	if (IsValid())
		return (CString)(LPCSTR)m_xmldoc->Getxml();
	else
		return _T("");
}
...
{
	return (CString)(LPSTR)m_xmldoc->Geturl();
}
...
{
	if (IsValid())
		return (CString)(LPCSTR)m_xmlnode->GetbaseName();
	return _T("");
}
...
{
	if (IsValid())
		return (CString)(LPCSTR)m_xmlnode->Gettext();
	else
		return _T("");
}
...
CString CXmlNodeWrapper::GetAttribName(int index)
{
	if (IsValid())
	{
		...
		if (attribs)
		{
			...
			if (attrib)
				return (CString)(LPCSTR)attrib->Getname();
		}
	}
...
CString CXmlNodeWrapper::GetAttribVal(int index)
{
	if (IsValid())
	{
		...
		if (attribs)
		{
			MSXML2::IXMLDOMAttributePtr attrib = attribs->Getitem(index);
			if (attrib)
				return (CString)(LPCSTR)attrib->Gettext();
		}
	}
...
{
	if (IsValid())
		return (CString)(LPCSTR)m_xmlnode->GetnodeTypeString();
	return _T("");
}
